...
```bash
bin/mirador-doctor                              # full health scan
gcloud container clusters list                  # is the cluster even there?
kubectl get pods -A --field-selector=status.phase!=Running
```

If `clusters list` is empty → `bin/demo-up.sh` hasn't run or failed.
...
2. **Terraform state lock.** A previous `bin/demo-up.sh` crashed
...
- **State lock** — delete the lock object (only if no other
  ...
  ```
- **budget-kill aftermath** — check `bin/budget.sh status` and verify
  if the cap needs raising. If yes: `bin/budget.sh set 20`. Then
  re-run `bin/demo-up.sh`.
...
```bash
bin/demo-down.sh     # idempotent, destroys whatever's there
bin/demo-up.sh       # recreate from scratch — 8 min cold start
```

If `bin/demo-up.sh` still fails with the same error after teardown,
