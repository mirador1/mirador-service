...
**Triggered by** : `MiradorServiceLatencyP99SLO` page or ticket alert.
...
4. **Look at the trace span breakdown** : where is the time spent ?
   ...
   - GC pause / suspension between spans → JVM heap pressure.
...
| All requests slower by ~constant ms | DB pool exhausted, queue forming | Hikari metrics : `active`, `pending` |
| Kafka `request-reply` span > 500ms | Consumer rebalance, broker overload | `kafka-consumer-groups.sh --describe` |
| Sawtooth pattern in p99 | Major GC pauses | JVM `gc_pause_seconds` metric, consider G1GC tuning |
