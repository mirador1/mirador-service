#!/usr/bin/env bash
# moved 2026-04-22 from bin/cluster/pf-prod.sh — per ~/.claude/CLAUDE.md subdirectory hygiene
# =============================================================================
# bin/cluster/port-forward/prod.sh — one kubectl port-forward per service to reach the GKE demo.
#
# Port scheme (three-environment policy, decided 2026-04-18):
#   Local compose     = upstream ports          (e.g. backend 8080)
#   Local kind         = upstream + 10000       (e.g. backend 18080)  — bin/cluster/port-forward/kind.sh
...
#   bin/cluster/port-forward/prod.sh --daemon      # background, logs in /tmp/pf-prod-*.log
#
...
if $DAEMON; then
  ...
  echo "   bin/cluster/port-forward/status.sh   list + test connectivity"
  echo "   bin/cluster/port-forward/stop.sh     tear everything down"
  exit 0
fi

# Foreground: wait forever; Ctrl-C triggers trap which kills all children.
trap 'echo ""; echo "⏹   stopping tunnels…"; bin/cluster/port-forward/stop.sh >/dev/null 2>&1; exit 0' INT TERM
echo ""
