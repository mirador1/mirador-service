...
> **Iris — observability-first showcase across 7 facets**
>
...
>
> - <span style="color: #ff2d20">●</span> **Observability** (the central axis) — OpenTelemetry traces + logs + metrics → LGTM stack (Tempo / Loki / Mimir / Grafana) + 3 SLOs as code via Sloth (availability / latency / enrichment) + multi-burn-rate alerting + 4 dashboards (SLO overview, Apdex, latency heatmap, SLO breakdown by endpoint) + chaos-driven SLO demo annotations + 3 runbooks + monthly review cadence + RTO measured = 7s on GKE Autopilot postgres pod-kill (target 30s).
> - <span style="color: #ff9500">●</span> **Infra · Cloud** — GKE production cluster `iris7-prod` (europe-west1) + Terraform IaC + multi-cloud deploy targets (AKS, EKS, Cloud Run, Fly.io — manual jobs in CI) + cert-manager + ingress-nginx + Argo CD GitOps + ephemeral pattern (ADR-0022) targeting ≤ €2/month idle + Workload Identity Federation (no service account JSON keys) + budget alerts via `bin/budget/budget.sh`.
> - <span style="color: #ffcc00">●</span> **Security** — JWT HS256 (15 min, refresh-token rotation) + X-API-Key static fallback + OAuth2/OIDC (Auth0 prod / Keycloak dev) + RBAC (`ROLE_ADMIN` / `ROLE_USER`) + Bucket4j rate-limit (100 req/min/IP) + IdempotencyFilter (POST/PATCH) + SecurityHeadersFilter (CSP/HSTS/X-Frame-Options) + per-MR security gates : grype + trivy + cosign sign+verify + dockle + OWASP dependency-check + secret-detection + semgrep-sast — all green.
> - <span style="color: #34c759">●</span> **CI · CD** — GitLab CI 19+ jobs across `lint / test / integration / k8s / package / sonar / native / compat / deploy` stages + compat matrix SB3/SB4 × Java17/21/25 (5 combos) + Conventional Commits enforced (Lefthook + commitlint) + auto-merge with `--remove-source-branch=false` + cosign sign+verify + SBOM (syft) + Renovate weekly dependency bumps + workflow `changes:` allowlist.
> - <span style="color: #30c9d4">●</span> **Architecture** — Hexagonal Lite (ADR-0044, `port/` only when cross-feature coupling emerges) + Feature-slicing (ADR-0008) + polyrepo flat α submodules (ADR-0060) + per-method MCP `@Tool` exposure (ADR-0062, "produces vs accesses" rule) + Clean Code 7 non-negotiables (binding, audited at `docs/audit/clean-code-architecture-*.md`) + 60+ Architecture Decision Records.
