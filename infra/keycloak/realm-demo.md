...
| Field | Value | Why |
|-------|-------|-----|
| `realm` | `customer-service` | Referenced by `application.yml` → `keycloak.issuer-uri` and by the Spring Security resource server JWT validator |
...
| `directAccessGrantsEnabled` | `true` | Enables the ROPC grant: `POST /realms/customer-service/protocol/openid-connect/token` with `grant_type=password` |
...
curl -s -X POST http://localhost:9090/realms/customer-service/protocol/openid-connect/token \
  -d "grant_type=password&client_id=customer-service&username=admin&password=admin-password" \
  | python3 -m json.tool
```
