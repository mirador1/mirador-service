...
| `cors-proxy.conf`               | Nginx config for a CORS-enabled reverse proxy in front of Loki (port 3100) and the Docker API. The Angular UI at `localhost:4200` queries both from the browser; neither supports CORS natively. | `/etc/nginx/conf.d/default.conf` in the `cors-proxy` container |
